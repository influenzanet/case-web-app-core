...
import { linkResolverRootUrl } from "./components/LinkResolver/Resolvers/LinkResolverConst";
...
const Pages: React.FC<PagesProps> = (props) => {
  ...
  // Loading page:
  const loadingDiv = (
    <div className={clsx(containerClassName, "my-3")}>
      <LoadingPlaceholder
  ...
  const notFoundRoute = props.defaultRoutes.notFound
    ? props.defaultRoutes.notFound
    : isAuth
    ? props.defaultRoutes.auth
    : props.defaultRoutes.unauth;
      ...
      <div>
        <Switch>
          <Redirect
            from="/"
            exact
            to={isAuth ? props.defaultRoutes.auth : props.defaultRoutes.unauth}
          />
          ...
          <Route
            path={props.defaultRoutes.surveyPage + "/:studyKey"}
            render={() => (
              <SurveyPage
          ...
          <Route
            path={linkResolverRootUrl}
            render={() => <LinkResolver defaultRoutes={props.defaultRoutes} />}
          />
