...
import SubmitSuccessWithLoginOptionsDialog, {
  LoginOptions,
} from "./Dialogs/SubmitSuccessWithLoginOptionsDialog";
...
const SurveyPage: React.FC<SurveyPageProps> = (props) => {
  ...
  const isLoggedIn =
    authState && authState.accessToken && authState.accessToken.length > 0
      ? true
      : false;
  ...
  const [currentSurveyKey, setCurrentSurveyKey] = useState<string>(
    initialSurveyKey ? initialSurveyKey : ""
  );
  const [currentSurveyResponse, setCurrentSurveyResponse] =
    useState<SurveyResponse | undefined>(undefined);
  const [tempParticipant, setTempParticipant] =
    useState<TempParticipant | undefined>();
  ...
  const currentSurveyName = getLocalizedString(
    currentSurvey?.surveyDef.props?.name,
    i18n.language
  );
  ...
  const getNextAction = (resp: any) => {
    ...
    if (!resp.data.surveys || resp.data.surveys.length < 1) {
      ...
      for (const survey of resp.data.surveys) {
        if (
          survey.category === "immediate" &&
          survey.surveyKey !== currentSurveyKey
        ) {
          const now = Math.round(new Date().getTime() / 1000);
          const currentlyValid =
            (!survey.validUntil || parseFloat(survey.validUntil) > now) &&
            (!survey.validFrom || parseFloat(survey.validFrom) < now);
          if (currentlyValid) {
  ...
  switch (contentState) {
    case "loading":
      pageContent = <LoadingPlaceholder color="white" minHeight="60vh" />;
      break;
    case "submitting":
      pageContent = <LoadingPlaceholder color="white" minHeight="60vh" />;
      break;
    ...
    <React.Fragment>
      ...
      <SubmitSuccessWithLoginOptionsDialog
        ...
        texts={{
          ...
          submitConfirm: t(
            "surveyPage:submitSuccessWithLoginOptionsDialog.successMsg"
          ),
          info: t("surveyPage:submitSuccessWithLoginOptionsDialog.info"), // '',
          loginBtn: t(
            "surveyPage:submitSuccessWithLoginOptionsDialog.loginBtn"
          ), // 'Login',
          registerBtn: t(
            "surveyPage:submitSuccessWithLoginOptionsDialog.registerBtn"
          ), // 'Register',
          withoutAccountBtn: t(
            "surveyPage:submitSuccessWithLoginOptionsDialog.withoutAccountBtn"
      ...
      <ProfileSelectionDialog
        ...
        texts={{
          ...
          manageProfiles: t(
            "surveyPage:profileSelectionDialog.manageProfilesBtn"
          ),
        }}
        avatars={avatars}
  ...
  return (
    <React.Fragment>
      <TitleBar content={currentSurveyName} showAlways={true} />
