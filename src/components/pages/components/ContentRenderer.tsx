...
import UserSymptomsHistory from "../../UserSymptomsHistory/UserSymptomsHistory";
...
const shouldHide = (hideWhen?: string, isAuth?: boolean): boolean => {
  if ((hideWhen === "auth" && isAuth) || (hideWhen === "unauth" && !isAuth)) {
    return true;
  }
  return false;
};
...
const ContentRenderer: React.FC<ContentRendererProps> = (props) => {
  ...
  const handleExtensionRendering = (
    ...
    if (!props.extensions) {
      console.warn(
        `No extension defined, so the following item is not rendered: ${JSON.stringify(
          item
        )}`
      );
      return null;
  ...
  const renderItem = (item: PageItem): React.ReactElement | null => {
    ...
    switch (item.config.type) {
      case "markdown":
        return (
          <MarkdownLoader
            ...
            markdownUrl={getTranslatedMarkdownPath(
              item.config.markdownUrl,
              i18n.language
            )}
      ...
      case "imageCard":
        ...
        return (
          <ImageCard
            ...
            className={item.className}
            imageSrc={
              item.config.imageSrc
                ? item.config.imageSrc
                : t(`${item.itemKey}.imageSrc`)
            }
            imageAlt={t(`${item.itemKey}.imageAlt`)}
            title={t(`${item.itemKey}.title`)}
            body={t(`${item.itemKey}.body`)}
            openActionText={
              item.config.showActionBtn
                ? t(`${item.itemKey}.actionLabel`)
                : undefined
            }
            onClick={() => {
              if (!action) {
              ...
              if (action.type === "openDialog") {
                dispatch(
                  dialogActions.openDialogWithoutPayload({ type: action.value })
                );
      ...
      case "actionCard":
        ...
        return (
          <ActionCard
            ...
            title={t(`${item.itemKey}.title`)}
            actionBtnText={
              actionCardAction ? t(`${item.itemKey}.actionBtn`) : undefined
            }
            footerText={
              item.config.useFooterText
                ? t(`${item.itemKey}.footer`)
                : undefined
            }
            image={item.config.image}
            ...
            onClick={() => {
              ...
              if (actionCardAction.type === "openDialog") {
                dispatch(
                  dialogActions.openDialogWithoutPayload({
                    type: actionCardAction.value,
                  })
                );
          ...
          >
            {item.config.hideBodyContent ? null : (
              <MarkdownRenderer markdown={t(`${item.itemKey}.body`)} />
            )}
          </ActionCard>
        );
      case "image":
      ...
      case "video":
        return (
          <VideoPlayer
            ...
            posterUrl={
              item.config.posterUrlKey
                ? t(`${item.itemKey}.${item.config.posterUrlKey}`)
                : undefined
            }
            ...
            fallbackText={
              item.config.fallbackTextKey
                ? t(`${item.itemKey}.${item.config.fallbackTextKey}`)
                : undefined
            }
          />
        );
      case "loginCard":
        return (
          <LoginCard
            ...
            title={t(`${item.itemKey}.title`)}
            infoText={
              item.config.showInfoText ? t(`${item.itemKey}.info`) : undefined
            }
            ...
            passwordInputPlaceholder={t(
              `${item.itemKey}.passwordInputPlaceholder`
            )}
      ...
      case "accordionList":
        const items = t(`${item.itemKey}`, { returnObjects: true }) as Array<{
          title: string;
          content: string;
        }>;
        if (items === undefined || !Array.isArray(items)) {
          return (
            <span key={item.itemKey}>
              Accordion items not found for: {item.itemKey}
            </span>
          );
        }
        return (
          <div key={item.itemKey} className={item.className}>
            <AccordionList
              itemKey={item.itemKey}
              items={items}
      ...
      case "deleteAccount":
        return <DeleteAccount key={item.itemKey} itemKey={item.itemKey} />;
      case "logoCredits":
        return (
          <LogoCredits
            key={item.itemKey}
            ...
            title={
              item.config.useTitle ? t(`${item.itemKey}.title`) : undefined
            }
          />
        );
      case "linkList":
      ...
      case "userSymptomsHistory":
        return (
          <UserSymptomsHistory
            key={item.itemKey}
            studyId={item.config.studyId}
            dateLocales={props.dateLocales}
            className={item.className}
          ></UserSymptomsHistory>
        );
      ...
      case "placeholder":
        return (
          ...
          >
            <h1 className="fs-1 text-center text-white text-uppercase m-0 p-2">
              {item.config.label}
            </h1>
          </div>
        );
      case "helmet":
        return (
          <Helmet key={item.itemKey}>
            {item.config.updateTitle ? (
              <title>{t(`${item.itemKey}.title`)}</title>
            ) : null}
            {item.config.updateDescription ? (
              <meta
                name="description"
                content={t(`${item.itemKey}.description`)}
              />
            ) : null}
          </Helmet>
        );
      ...
      case "extension":
        return handleExtensionRendering(item, renderItem, (url: string) => {
          history.push(url);
        });
      case "router":
        const dRoutes = item.config.pagesConfig.defaultRoutes
          ? item.config.pagesConfig.defaultRoutes
          : BasicRoutes;
  ...
  const renderColumn = (col: PageColumn, index: number) => {
    ...
    return (
      <div className={col.className} key={col.key ? col.key : index.toFixed()}>
  ...
  return (
    <React.Fragment>
      ...
      {props.hideTitleBar ? null : <TitleBar content={t("title")} />}
      {props.rows.map((row) => {
        ...
        <Switch key={props.pageKey}>
          ...
          {props.subPages?.defaultRoutes !== undefined ? (
            <Redirect
              to={
                props.isAuthenticated
                  ? props.subPages.defaultRoutes.auth
                  : props.subPages.defaultRoutes.unauth
              }
            />
          ) : null}
        </Switch>
