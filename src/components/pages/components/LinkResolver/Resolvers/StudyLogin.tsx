...
const StudyLogin: React.FC<StudyLoginProps> = (props) => {
  ...
  const accessToken = useSelector(
    (state: RootState) => state.app.auth?.accessToken
  );
  ...
  useEffect(() => {
    ...
    validateToken(token).then((stayOnPage) => {
      if (stayOnPage) {
  ...
  const validateToken = async (token: string | null): Promise<boolean> => {
    ...
    setLoading(true);
    try {
      const response = await autoValidateTemporaryTokenReq(
        token,
      ...
      if (response.status === 200) {
        if (hasToken) {
          if (loggedInUser !== response.data.accountId) {
            ...
            let navTo = props.defaultRoutes.studyPage
              ? props.defaultRoutes.studyPage
              : props.defaultRoutes.auth;
    ...
    <div style={{ width: 450 }}>
      <LoginCard
        ...
        emailInputPlaceholder={t(
          `${translationRootKey}.content.emailInputPlaceholder`
        )}
        passwordInputLabel={t(
          `${translationRootKey}.content.passwordInputLabel`
        )}
        passwordInputPlaceholder={t(
          `${translationRootKey}.content.passwordInputPlaceholder`
        )}
        rememberMeLabel={t(`${translationRootKey}.content.rememberMeLabel`)}
        loginBtn={t(`${translationRootKey}.content.btn`)}
        passwordForgottenBtn={t(
          `${translationRootKey}.content.passwordForgottenBtn`
        )}
