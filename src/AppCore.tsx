...
import { handleOpenExternalPage } from "@influenzanet/case-web-ui";
...
const AppCore: React.FC<AppCoreProps> = (props) => {
  ...
  const defaultRoutes = props.pagesConfig?.defaultRoutes
  ...
  useEffect(() => {
    const language = loadLastSelectedLanguage(
  ...
  useEffect(() => {
    dispatch(
      appConfig.updateInstanceID(
        props.appConfig?.instanceId ? props.appConfig?.instanceId : "default"
      )
    );
